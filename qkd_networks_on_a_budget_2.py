...
# what does v2 do? trying to optimize the workflow (order of loops, saving networks, etc)

Ns = [200]                    # list of network sizes
...
for N in Ns:
    ...
    # lets move in the budget folder to keep things tidy
    folder = os.path.join("budget_outputs", "N%d" % N)
    os.makedirs(folder, exist_ok=True)
    os.chdir(folder)

    for DV_budget in budget_list:      # not ideal that this is the outer loop, but easier to implement
        suffix = '_N%d' % N
        if qkd == 'hybrid':
            suffix += '_d_hyb%.0f' % d_hybrid + 'DVbudget%s' % str(DV_budget)
        suffix += '_NX'
        suffix += rho_span
        ...
        if os.path.exists(budg_filename):
            print('Skipping DV_budget=%s: %s already exists.' % (str(DV_budget), budg_filename))
            continue
        ...
        dict_of_num_DV_edges = {radius: [] for radius in radii}
        ...
        for it in range(n_iter):            # looping over different starting graphs
            print('N=%d, instance %d, DV_budget=%s'%(N,it, str(DV_budget)))
            ...
            rate_sum_accum = []
            rate_sq_sum_accum = []
            for radius in radii:            # same starting graph, looping over different system sizes --> different pruning
                # PRUNING
                A_pruned = np.zeros_like(A)
                W = np.zeros_like(A)                        # matrix of the graph weights ie the inverse rates (s/bit)
                W_CV = np.zeros_like(A)
                candidate_DV_edges = []                     # edge list of possible DV connections: it is a list of (i,j) tuples
                for i in range(N):
                    for j in range(i):
                        if A[i,j] == 1:
                            ...
                            if dij < d_c_DV and h_rate > rate_min:    # the transition CV/DV can cause problems, double-checking here
                                if np.random.uniform() > PoF * int(edges_may_fail):  # ie if edge doesnt fail (uniform returns 0<x<1 by default)
                                    ...
                                    if dij <= d_c_CV:
                                        W_CV[i,j] = W_CV[j,i] = h_rate**-1      # i need to build a CV-only graph to gradually add DV links to it 
                                    elif dij < d_c_DV:                          # if the edge is DV but not CV, then it is a candidate for being added to the CV graph
                ...
                dict_of_num_DV_edges[radius].append(number_possible_DV_edges)    # for the budget-related plot

                DV_edges = sorted_candidate_edges[:min(DV_budget, number_possible_DV_edges)]  # select the top DV_budget edges based on centrality
                G_budget = G_CV.copy()
                G_budget.add_edges_from(DV_edges)
                ...
                if len([v for v in dict(G_budget.degree).values() if int(v) > 1]) != 0:
                    average_clustering_coefficient = sum(clustering_coefficients.values()) / len([v for v in dict(G_budget.degree).values() if int(v) > 1])
                else:
                ...
                deg_hist = nx.degree_histogram(G_budget)           # degree histogram: it is a list, degree is the index
                ...
                if compute_nw_rates:
                    ...
                    while node_counter < node_max:
                        ...
                        for source in range(target):
                            if nx.has_path(G_budget, source, target):   # could i use 'weights' dict keys to check this?
        ...
        # NUMBER OF DV EDGES AVAILABLE FOR THE HYBRID PROTOCOL
        np.savetxt('out_'+qkd+'_rhos_budg' + suffix + '.dat', np.vstack((rhos, [np.average(dict_of_num_DV_edges[radius]) for radius in radii])) )
